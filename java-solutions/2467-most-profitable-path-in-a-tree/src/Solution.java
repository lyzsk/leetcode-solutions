...
public class Solution {
    ...
    private boolean fillBobPath(int node, int parentNode, ArrayList<Integer> path,
    ...
    private int getAliceMaxScore(int node, int parentNode, int currScore, int[] bobPath,
        ArrayList<Integer>[] graph, int timestamp, int[] amount) {
        ...
        for (int neighborNode : graph[node]) {
            if (neighborNode != parentNode) {
                maxScore = Math.max(maxScore,
                    getAliceMaxScore(neighborNode, node, currScore, bobPath, graph, timestamp + 1,
                        amount));
            }
        }
        return maxScore;
